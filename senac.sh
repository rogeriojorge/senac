...
runREGCOILoriginal=1;    #run REGCOIL for original/VMEC file
...
nthetaM=50;              #resolution in theta for fit and Mercier's coordinates
nphiM=70;                #resolution in phi for fit and Mercier's coordinates
...
nPlotTheta=60;           #number of interpolating points in theta
nPlotPhi=110;            #number of interpolating points in phi
plotPointsFig=50;        #plotpoints for 3D figure
maxRecursPlot=2;         #max recursion for 3D figure
ImageSizePlot=600;       #image size for 3D figure
...
nthetapointsBsurface=20; #plot points in theta for magnetic field on surface
...
if ( [[ $plotRegcoilOriginal == 1 ]] ); then
	...
	plotRegcoilSENAC=1;
	wolframscript -noprompt -script src/plot_regcoil.wls $proj $ordern $nModes $ImageSizePlot $ImageResolutionPlot $coilsPerHalfPeriod $numHalfPeriodsToPlot $plotRegcoilSENAC $npointsContourPlotREGCOIL $npointsInterpCoilPosREGCOIL $interpOrderCoilPosREGCOIL $plotPointsFig $maxRecursPlot | tee data/${proj}/senac_${proj}_output.txt
fi
